...
function useToken() {
  ...
  const getContract = (): ethers.Contract => {
    return new ethers.Contract(
      ...
      [
        // hasClaimed mapping
        "function hasClaimed2(address) public view returns (bool)",
        // Claim function
  ...
  const getClaimedStatus = async (address: string): Promise<boolean> => {
    ...
    return await token.hasClaimed2(address);
  };
  ...
  const claimAirdrop = async (): Promise<void> => {
    ...
    try {
      const tx = await token.claim2(formattedAddress, numTokens, proof);
      await tx.wait(1);
      await syncStatus();
    } catch (e) {
